...
## 0. AI 管理转型路线图（惠居上海 · 业务视角）

> **依据**：[docs/meeting/AI应用于管理模式规划 (1).txt](../meeting/AI应用于管理模式规划%20(1).txt) 及 `docs/` 下画像系列文档。  
> **目的**：在本文件原有「技术模块」之上，补充 **A 点（传统人治）→ B 点（数据 + AI 治理）** 的阶段叙事，与技术优先级 **对照执行**。

### 0.1 项目愿景（会议精神）

- **半年内**：管理模式从 **依赖述职与一次性拉表**，转向 **末端行为在线、可量化、可干预**，并力争对外讲清 **2～3 个 AI 管理提效故事**。
- **核心目标**：**管到每个人、每套房、每个关键指标** — 与 [租务管家画像深度分析报告.md](../租务管家画像深度分析报告.md)、[资管经理画像深度分析报告.md](../资管经理画像深度分析报告.md)、[房源画像深度分析报告.md](../房源画像深度分析报告.md) 一致。
- **对标**：美团/滴滴 — **响应率、完成率、过程数据** 替代纯结果问责（机制与合规边界见 [研讨关键问题.md](../研讨关键问题.md)）。

### 0.2 阶段规划（与画像 Phase 对齐）

| 阶段 | 时间（建议） | 业务侧重 | 关键产出 |
|------|----------------|----------|----------|
| **第一阶段：写剧本** | 第 1～2 月 | 场景冻结与 Demo | 场景文档 + 原型；三主线：**资管末端监控**、**管家实时看板**、**房源评价体系** |
| **第二阶段：开灯** | 第 2～3 月 | 可观测 | NFC 签到 **试点**、GPS 围栏、**实时状态看板**（在房/在途等 — 隐私见研讨 Q5） |
| **第三阶段：建表** | 第 3～4 月 | 指标与预警 | 系数 A/B/C 与 **AM 通排** 生产级计算、经分看板、**低绩效预警** 名单对接 |
| **第四阶段：自动** | 第 4～6 月 | AI 驱动 | 派单/调度试点、**AI 绩效辅导**（录音评分，对齐制度 3.0）、风险房源与问题员工 **预测干预** |
...
|------|------|------|
| 人力 | 人事系统 | 职级、在岗、应出勤 |
| 法务/合规 | 制度与协议 | 位置、录音、舆情采集边界 |
| 财务 | 费控/结算 | 沙盘与激励试点核对 |
| 业务 | 收房、签约、房源主数据 | 资管/管家指标 |
| 客服 | 客诉 | 有责客诉门槛 — [metrics.md](metrics.md) |
| 飞书 | 审批、机器人 | 待办、日报、通知 |

### 0.4 与技术章节的关系

- **§1～§8** 仍为 **技术实现层**（模块、接口、优先级），开发排期在 **不突破合规与数据可用性** 的前提下，向 **§0.2** 的阶段目标靠拢。
- 详细研讨清单：[docs/研讨关键问题.md](../研讨关键问题.md)。
